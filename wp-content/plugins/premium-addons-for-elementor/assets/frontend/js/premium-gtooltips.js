		...
		var premiumGlobalTooltipsHandler = function ($scope, $) {
			...
			function generateGlobalTooltip() {
				...
				var $tooltipTarget =
					"" !== settings.target
						? $scope
								.attr("data-tooltip-content", "#tooltip_content-" + id)
				...
				if (elementorFrontend.isEditMode()) {
					...
					var hasInteractiveElements =
						$("#tooltip_content-" + id).find("a, button").length > 0;

					var newOptions = {
						functionBefore: function () {
							if (
								settings.hideOn.includes(
						...
						functionReady: function (origin, tooltipObj) {
							...
							$(".tooltipster-box").addClass(
								"tooltipster-box-" + settings.elemID,
							);
							...
							if ("template" === settings.type) {
								var templateID = $("#tooltip_content-" + id).data(
									"template-id",
								);
									...
									$.ajax({
										...
										if (undefined !== data.template_content) {
											...
											hasInteractiveElements =
												$(data.template_content).find("a, button").length > 0;
											...
											if ("" !== settings.target) {
												$scope
													.find(settings.target)
													.tooltipster("content", data.template_content);
											} else {
							...
							$scope
								.find(".premium-global-tooltips-wrapper-temp-" + id)
								.remove();
							...
							if ("lottie" === type) {
								...
								var isChromeDesktop =
									["widescreen", "desktop", "laptop"].includes(
										elementorFrontend.getCurrentDeviceMode(),
									) &&
								...
								if (isChromeDesktop) {
									$(".tooltipster-box-" + id)
										.closest(".premium-tooltipster-base")
										.addClass("has-lottie-animation");
								}

								var $tooltipContent = $(tooltipObj.tooltip);

								var lottieInstance = new premiumLottieAnimations(
									$tooltipContent,
								);
								lottieInstance.init();
							}
						...
						updateAnimation: settings.anime, // fix: weird wiggle @see https://github.com/calebjacob/tooltipster/issues/795
						...
						side:
							"string" === typeof settings.side
								? cleanPosition(settings.side)
								: settings.side[0], // does this return anything except for a string?
					...
					Object.keys(newOptions).forEach(function (key) {
						if ("" !== settings.target) {
							$scope
								.find(settings.target)
								.tooltipster("option", "" + key, newOptions[key]);
						} else {
				...
				if ("gallery" === settings.type && 1 < settings.content.length) {
					...
					$scope.on(event, function () {
						var state =
							"" !== settings.target
								? $scope.find(settings.target).tooltipster("status").state
								: $scope.tooltipster("status").state;
				...
				if (settings.follow_mouse) {
					$scope.on("mousemove.paTooltipsFollower" + id, function (e) {
						var element = $(".tooltipster-box-" + id).closest(
							".premium-tooltipster-base",
						);

						$(element).css({
							transition:
								"left 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94) 0s, top 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94) 0s",
			...
			function getTooltipsterSettings(settings) {
				...
				var tooltipSettings = {
					functionBefore: function () {
						if (
							settings.hideOn.includes(elementorFrontend.getCurrentDeviceMode())
						) {
							return false;
						}
					...
					functionInit: function (instance, helper) {
						...
						if (!elementorFrontend.isEditMode()) {
							...
							$(
								".pa-tooltipstered.premium-global-tooltips-wrapper-temp-" + id,
							).remove();
						}
						...
						if ("" !== settings.uniqueClass) {
							if ("" !== settings.target) {
								$scope
									.find(settings.target)
									.attr("pa-tooltip-selector", settings.uniqueClass);
							} else {
					...
					updateAnimation: settings.anime,
					contentAsHTML: true,
					contentCloning: true,
					plugins: ["sideTip"],
					animation: settings.anime,
			...
			function addTourEvents() {
				if ("" !== settings.uniqueClass) {
						...
						for (var i = 0; i < classes.length; i++) {
							...
							if ("tooltip-" === triggerId.slice(0, 8)) {
								...
								$(trigger).on("click.paTourTrigger", function (e) {
									...
									$(
										".tooltipstered[pa-tooltip-selector=" + classes[i] + "]",
									).tooltipster("open");
								});
					...
					$(".premium-tooltipster-closer").on("click.paCloseTour", function () {
						closeTooltips(false);
					});
				}
			}
			...
			function updateGalleryContent(src) {
				var randSrc = getRandomImage(src),
					newContent =
						...
						id +
						'" class="premium-global-tooltip-content premium-tooltip-content-wrapper-' +
						id +
						'"><span class="premium-tooltip-gallery"><img src="' +
						randSrc +
						'"></span></div></div>';
			...
			function closeTooltips(target) {
				...
				$.each(instances, function (i, instance) {
					if (target) {
						...
						if (
							!instance._$origin.hasClass("premium-image-hotspots-main-icons")
						) {
							instance.close();
						}
					}
