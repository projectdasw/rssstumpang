	...
	$(window).on("elementor/frontend/init", function () {
		var premiumShapeDividerHandler = function ($scope, $) {
			...
			if (!isEditMode) {
				...
				var selectedShape = $scope
					.find("#premium-shape-divider-" + id)
					.data("shape");

				if (selectedShape) renderShapeSVG(selectedShape);
			}
			...
			function setCustomFill() {
				// set the fill viewBox
				var shapeViewBox = $("#premium-shape-divider-" + id + " svg").attr(
					"viewBox",
				);

				$("#premium-shape-divider-fill-" + id).attr("viewBox", shapeViewBox);
				// set the fill value
				$("#premium-shape-divider-" + id + " svg").attr(
					"fill",
					"url(#pa-shape-divider-fill-" + id + ")",
				);
			}

			if (isCustomSvg) {
				$scope
					.find("#premium-shape-divider-" + id + " svg")
			...
			function updateShapePosition() {
				var shapePos = getComputedStyle($scope[0]).getPropertyValue(
					"--pa-sh-divider-pos",
				);
				$scope
					.removeClass(
						"premium-shape-divider__top premium-shape-divider__bottom premium-shape-divider__right premium-shape-divider__left",
					)
					.addClass("premium-shape-divider__" + shapePos);
			}

			function updateShapeSize() {
				var adjustSize =
					$scope.hasClass("premium-shape-divider__left") ||
					$scope.hasClass("premium-shape-divider__right");
				...
				if (adjustSize) {
					...
					$scope
						...
						.css({
							width: "103%",
							transform: $scope.hasClass("premium-shape-divider__right")
								? "rotate(-90deg) translate(0,-100%)"
								: "rotate(90deg) translate(0,-100%)",
							width: containerHeight,
						})
						.get(0)
						.style.setProperty("--premium-shape-divider-h", containerHeight); // this needs to add for each devices.
				} else {
					$scope
						.find("#premium-shape-divider-" + id + " svg")
						.attr("style", "");
			...
			function checkWaveAnimation() {
				...
				if (hasWaveAnime) {
					...
					var animeSettings = {
						...
						easing: "linear",
						duration: 7500,
						d: [{ value: wave2 }, { value: wave3 }, { value: wave1 }],
					};

					$scope
						.find("#premium-shape-divider-" + id + " svg")
						.attr("viewBox", "0 0 1440 320");
