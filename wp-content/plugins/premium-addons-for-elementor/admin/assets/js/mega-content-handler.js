	...
	$(document).ready(function () {
		window.PaMegaContentHandler = {
			initEvents: function () {
				...
				$(".premium-live-editor-iframe-modal .eicon-close").on(
					"click",
					this.closeModal,
				);
				...
				$(document).on(
					...
					function (e) {
						if (
							$(e.target).closest(".dialog-lightbox-widget-content").length < 1
						) {
							_this.closeModal();
						}
					},
			...
			closeModal: function (inserted = false) {
				...
				if (!inserted) {
					var tempId = $(
						".premium-live-editor-iframe-modal #pa-live-editor-control-iframe",
					).attr("data-premium-temp-id");
